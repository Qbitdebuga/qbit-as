...
export class AuthService {
  ...
  constructor(
    ...
    this.serviceId = this.configService.get<string>('SERVICE_ID', 'ar');
    this.serviceName = this.configService.get<string>('SERVICE_NAME', 'Accounts Receivable Service');
   ...
   */
  async getServiceToken(options: ServiceTokenOptions = { scopes: ['ar:read', 'ar:write'] }): Promise<string> {
    try {
   ...
   */
  async getAuthHeaders(options: ServiceTokenOptions = { scopes: ['ar:read', 'ar:write'] }): Promise<{ Authorization: string }> {
   ...
   */
  async refreshToken(options: ServiceTokenOptions = { scopes: ['ar:read', 'ar:write'] }): Promise<string> {
    this.token = null;
