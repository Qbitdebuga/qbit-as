...
export default function BillPaymentApplication({
  ...
  const handleApplyPayment = async () => {
    ...
    const hasInvalidAmounts = selectedBillIds.some(
      (billId) => !amounts[billId] || amounts[billId] <= 0,
    );
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Select bills to apply this payment to</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-4 p-4 bg-muted rounded-md">
          <div className="flex justify-between">
            ...
            <div>
              ...
              <div
                className={`text-2xl font-bold ${remainingAmount < 0 ? 'text-red-500' : remainingAmount === 0 ? 'text-green-600' : ''}`}
              >
              ...
              {bills.map((bill) => {
                ...
                return (
                  <TableRow key={billId} className={isPaid ? 'opacity-50' : ''}>
                    ...
                    <TableCell>
                      <Badge className={billStatusColorMap[bill.status]}>{bill.status}</Badge>
                    </TableCell>
