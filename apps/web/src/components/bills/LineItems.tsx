...
// Define a local bill line item type
interface SimpleBillLineItem {
  id: string | number;
  description: string;
  quantity: number;
  unitPrice: number;
...
interface LineItemsProps {
  lineItems: SimpleBillLineItem[];
  setLineItems: React.Dispatch<React.SetStateAction<SimpleBillLineItem[]>>;
}
...
export default function LineItems({ lineItems, setLineItems }: LineItemsProps) {
  ...
  const updateLineItem = (index: number, field: keyof SimpleBillLineItem, value: any) => {
    const updatedItems = [...lineItems];
    updatedItems[index] = {
      ...updatedItems[index],
      [field]: value
