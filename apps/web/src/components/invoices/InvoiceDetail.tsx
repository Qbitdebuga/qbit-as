...
export default function InvoiceDetail({ invoice, onStatusChange }: InvoiceDetailProps) {
  ...
  const canRecordPayment = [InvoiceStatus.PENDING, InvoiceStatus.SENT, InvoiceStatus.PARTIAL, InvoiceStatus.OVERDUE].includes(invoice.status);
  const canVoid = [InvoiceStatus.DRAFT, InvoiceStatus.PENDING, InvoiceStatus.SENT, InvoiceStatus.PARTIAL, InvoiceStatus.OVERDUE].includes(invoice.status);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <div className="flex items-center mt-2 space-x-2">
            ...
            {invoice.customerReference && (
              <span className="text-sm text-gray-500">Customer Ref: {invoice.customerReference}</span>
            )}
          </div>
        </div>
        <div className="flex space-x-2">
          ...
          {canFinalize && (
            <Button variant="default" onClick={handleFinalize}>Finalize</Button>
          )}
          {canMarkAsSent && (
            <Button variant="default" onClick={handleMarkAsSent}>Mark as Sent</Button>
          )}
          {canRecordPayment && (
            <RecordPaymentDialog 
          ...
          {canVoid && (
            <Button variant="outline" onClick={handleVoid}>Void</Button>
          )}
