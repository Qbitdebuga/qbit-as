...
export class GeneralLedgerClient implements IGeneralLedgerService {
  ...
  async getJournalEntries(page = 1, limit = 20): Promise<PaginatedResponseDto<JournalEntry>> {
    return this.request<PaginatedResponseDto<JournalEntry>>(
  ...
  async getBalanceSheetSummary(asOfDate?: string): Promise<BalanceSheetSummaryDto> {
    ...
    return this.request<BalanceSheetSummaryDto>(
      `/financial-statements/balance-sheet/summary${query}`,
    );
  }

  async getIncomeStatementSummary(
    startDate?: string,
    endDate?: string,
    ...
    return this.request<IncomeStatementSummaryDto>(
      `/financial-statements/income-statement/summary${query}`,
    );
  }
